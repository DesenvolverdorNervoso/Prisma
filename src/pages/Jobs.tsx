...
export const Jobs: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-dark-card p-4 rounded-lg border border-slate-200 dark:border-dark-border relative">
          <Search className="h-4 w-4 text-gray-400 dark:text-dark-muted absolute mt-3 ml-3" />
          ...
          {activeTab === 'reqs' && (
             <div className="animate-in fade-in">
                <FormSection title="Requisitos da Vaga">
                   ...
                   <div className="bg-primary-50/40 dark:bg-slate-900/40 p-2 rounded-md space-y-2 max-h-60 overflow-y-auto border border-slate-100 dark:border-dark-border">
                      {formData.requirements_list?.map((req, i) => (
                        <div key={req.id} className="flex items-center gap-2 bg-white dark:bg-dark-bg p-2 border border-slate-200 dark:border-dark-border rounded shadow-sm">
                   ...
                   <tbody>
                     {jobCandidates.map(jc => (
                       <TableRow key={jc.id}>
                         ...
                         <TableCell>
                           <select 
                             className="text-sm border rounded p-1 bg-white dark:bg-dark-bg dark:border-dark-border dark:text-dark-text"
                             value={jc.status}
